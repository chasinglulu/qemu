...
#define SIGIE_VIRT_ACPU_TYPE ARM_CPU_TYPE_NAME("cortex-a78ae")

static bool sigi_virt_get_virt(Object *obj, Error **errp)
{
    SigiEVirt *s = SIGIE_VIRT(obj);
...
static void create_gpio(SigiEVirt *s, int gpio)
{
...
static void create_uart(SigiEVirt *s, int uart)
{
    MemoryRegion *sysmem = get_system_memory();
...
static void create_usb(SigiEVirt *s, int usb)
{
    MemoryRegion *sysmem = get_system_memory();
...
static void create_sdhci(SigiEVirt *s, int sdhci)
{
    MemoryRegion *sysmem = get_system_memory();
...
static void create_gic(SigiEVirt *s)
{
    MemoryRegion *sysmem = get_system_memory();
    ...
    for (i = 0; i < nr_apu; i++) {
        ...
        int ppibase = SIGIE_VIRT_NUM_IRQS + i * GIC_INTERNAL + GIC_NR_SGIS;
        int irq;
...
static void create_pcie(SigiEVirt *s, int pcie)
{
    MemoryRegion *sysmem = get_system_memory();
    DeviceState *gicdev = DEVICE(&s->apu.gic);
...
static void create_apu(SigiEVirt *s)
{
    MemoryRegion *sysmem = get_system_memory();
    int i;
 ...
 */
static void create_ddr_memmap(SigiEVirt *s)
{
    uint64_t cfg_ddr_size = memory_region_size(s->cfg.mr_ddr);
    MemoryRegion *sysmem = get_system_memory();
    hwaddr low_base = base_memmap[VIRT_LOW_MEM].base;
    hwaddr low_size = base_memmap[VIRT_LOW_MEM].size;
    hwaddr high_base = base_memmap[VIRT_HIGH_MEM].base;
    hwaddr high_size = base_memmap[VIRT_HIGH_MEM].size;
    uint64_t offset = 0;
    char *name;
    uint64_t mapsize;

    mapsize = cfg_ddr_size < low_size ? cfg_ddr_size : low_size;
    name = g_strdup_printf("sigi-low-ddr");
    /* Create the MR alias.  */
    memory_region_init_alias(&s->mr_low_ddr, OBJECT(s),
                                name, s->cfg.mr_ddr,
                                offset, mapsize);
    g_free(name);

    offset += mapsize;
    cfg_ddr_size -= mapsize;
    if (cfg_ddr_size > 0) {
        mapsize = cfg_ddr_size < high_size ? cfg_ddr_size : high_size;
        name = g_strdup_printf("sigi-high-ddr");
        memory_region_init_alias(&s->mr_high_ddr, OBJECT(s),
                                    name, s->cfg.mr_ddr,
                                    offset, mapsize);
    }

    /* Map it onto the main system MR.  */
    memory_region_add_subregion(sysmem, low_base, &s->mr_low_ddr);
    memory_region_add_subregion(sysmem, high_base, &s->mr_high_ddr);
    g_free(name);

}

static void create_unimp(SigiEVirt *s)
{
...
static PFlashCFI01 *virt_flash_create1(SigiEVirt *s,
                                        const char *name,
...
static void create_cfi_flash(SigiEVirt *s)
{
...
static void create_flash_memmap(SigiEVirt *s)
{
    MemoryRegion *sysmem = get_system_memory();
    /*
...
static Property sigi_virt_properties[] = {
    DEFINE_PROP_LINK("sigie-virt.ddr", SigiEVirt, cfg.mr_ddr, TYPE_MEMORY_REGION,
                     MemoryRegion *),
    DEFINE_PROP_BOOL("has-emmc", SigiEVirt, cfg.has_emmc, false),
...
static const TypeInfo sigi_soc_info = {
    .name = TYPE_SIGIE_VIRT,
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(SigiEVirt),
