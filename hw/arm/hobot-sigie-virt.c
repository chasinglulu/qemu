...
#include "hw/arm/sigie-virt.h"
#include "qom/object.h"
#include "sysemu/sysemu.h"
#include "qemu/log.h"

#define TYPE_HOBOT_SIGIE_VIRT_MACHINE MACHINE_TYPE_NAME("hobot-sigie-virt")
OBJECT_DECLARE_SIMPLE_TYPE(HobotSigiEVirt, HOBOT_SIGIE_VIRT_MACHINE)

struct HobotSigiEVirt {
...
static void hobot_sigie_virt_set_emmc(Object *obj, bool value, Error **errp)
{
    HobotSigiEVirt *s = HOBOT_SIGIE_VIRT_MACHINE(obj);

    s->cfg.has_emmc = value;
}

static void hobot_sigie_virt_set_virt(Object *obj, bool value, Error **errp)
{
    HobotSigiEVirt *s = HOBOT_SIGIE_VIRT_MACHINE(obj);
...
static void hobot_sigie_virt_set_secure(Object *obj, bool value, Error **errp)
{
    HobotSigiEVirt *s = HOBOT_SIGIE_VIRT_MACHINE(obj);
...
static const CPUArchIdList *virt_possible_cpu_arch_ids(MachineState *ms)
{
    ...
    HobotSigiEVirt *vms = HOBOT_SIGIE_VIRT_MACHINE(ms);
...
static void create_fdt(HobotSigiEVirt *s)
{
...
static void fdt_add_timer_nodes(const HobotSigiEVirt *vms)
{
...
static void fdt_add_cpu_nodes(const HobotSigiEVirt *vms)
{
    int cpu;
    int addr_cells = 1;
...
static void create_pcie_irq_map(const HobotSigiEVirt *vms,
                                uint32_t gic_phandle,
...
static void fdt_add_pcie_node(HobotSigiEVirt *vms, int pcie)
{
    char *nodename;
...
static void fdt_add_gic_node(HobotSigiEVirt *vms)
{
    char *nodename;
...
static void fdt_add_clk_nodes(HobotSigiEVirt *vms)
{
...
static void fdt_add_aliases_nodes(HobotSigiEVirt *vms)
{
    int i;
...
static void fdt_add_usb_nodes(const HobotSigiEVirt *vms)
{
    char *nodename;
...
static void fdt_add_gpio_nodes(const HobotSigiEVirt *vms, int gpio)
{
...
static void fdt_add_sdhci_nodes(const HobotSigiEVirt *vms, int sdhci)
{
    char *nodename;
...
static void fdt_add_flash_node(const HobotSigiEVirt *vms, int flash)
{
...
static void fdt_add_uart_nodes(const HobotSigiEVirt *vms, int uart)
{
    char *nodename;
...
static void *hobot_sigie_virt_dtb(const struct arm_boot_info *binfo, int *fdt_size)
{
    const HobotSigiEVirt *board = container_of(binfo, HobotSigiEVirt,
                                                 bootinfo);
...
static void fdt_add_memory_nodes(HobotSigiEVirt *s, void *fdt, uint64_t ram_size)
{
    ...
    for (i = 0; i < ARRAY_SIZE(addr_ranges); i++) {
        addr_ranges[i].base = base_memmap[VIRT_LOW_MEM + i].base;
        addr_ranges[i].size = base_memmap[VIRT_LOW_MEM + i].size;
    }
...
static void hobot_sigie_virt_modify_dtb(const struct arm_boot_info *binfo,
                                    void *fdt)
{
    HobotSigiEVirt *s = container_of(binfo, HobotSigiEVirt, bootinfo);

    fdt_add_memory_nodes(s, fdt, binfo->ram_size);
}

static void hobot_sigie_virt_mach_done(Notifier *notifier, void *data)
{
    HobotSigiEVirt *vms = container_of(notifier, HobotSigiEVirt,
                                         machine_done);
    MachineState *ms = MACHINE(vms);
...
static void hobot_sigie_virt_mach_init(MachineState *machine)
{
    HobotSigiEVirt *vms = HOBOT_SIGIE_VIRT_MACHINE(machine);
    ...
    object_initialize_child(OBJECT(machine), "sigie-virt", &vms->soc,
                            TYPE_SIGIE_VIRT);
    object_property_set_link(OBJECT(&vms->soc), "sigie-virt.ddr",
    ...
    vms->bootinfo.loader_start = base_memmap[VIRT_LOW_MEM].base;
    vms->bootinfo.get_dtb = hobot_sigie_virt_dtb;
    vms->bootinfo.modify_dtb = hobot_sigie_virt_modify_dtb;
    vms->bootinfo.skip_dtb_autoload = true;
    vms->bootinfo.psci_conduit = vms->psci_conduit;
    arm_load_kernel(ARM_CPU(first_cpu), machine, &vms->bootinfo);

    vms->machine_done.notify = hobot_sigie_virt_mach_done;
    qemu_add_machine_init_done_notifier(&vms->machine_done);
}

static void hobot_sigie_virt_mach_instance_init(Object *obj)
{
    HobotSigiEVirt *vms = HOBOT_SIGIE_VIRT_MACHINE(obj);
    MachineState *ms = MACHINE(vms);

    ms->smp.cores = SIGIE_VIRT_CLUSTER_SIZE;
    ms->smp.clusters = SIGIE_VIRT_CLUSTERS;
}

static void hobot_sigie_virt_mach_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);

    mc->desc = "Horizon Robotics Jounery SigiE Development Board";
    mc->init = hobot_sigie_virt_mach_init;
    mc->min_cpus = SIGIE_VIRT_NR_ACPUS;
    mc->max_cpus = 6;
    mc->minimum_page_bits = 12;
    mc->possible_cpu_arch_ids = virt_possible_cpu_arch_ids;
    mc->default_cpus = SIGIE_VIRT_NR_ACPUS;
    mc->no_cdrom = 1;
    mc->no_sdcard = 1;
    mc->no_floppy = 1;
    mc->block_default_type = IF_EMMC;
    mc->default_ram_id = "sigie-virt.ddr";

    object_class_property_add_bool(oc, "emmc", NULL,
		            hobot_sigie_virt_set_emmc);
    object_class_property_add_bool(oc, "virt", NULL,
		            hobot_sigie_virt_set_virt);
    object_class_property_add_bool(oc, "secure", NULL,
		            hobot_sigie_virt_set_secure);
}

static const TypeInfo hobot_sigie_virt_mach_info = {
    .name       = TYPE_HOBOT_SIGIE_VIRT_MACHINE,
    .parent     = TYPE_MACHINE,
    .class_init = hobot_sigie_virt_mach_class_init,
    .instance_init = hobot_sigie_virt_mach_instance_init,
    .instance_size = sizeof(HobotSigiEVirt),
};

static void hobot_sigie_virt_machine_init(void)
{
    type_register_static(&hobot_sigie_virt_mach_info );
}

type_init(hobot_sigie_virt_machine_init)
