...
#define TYPE_HOBOT_AYERS_VIRT_MACHINE MACHINE_TYPE_NAME("hobot-ayers-virt")
OBJECT_DECLARE_SIMPLE_TYPE(HobotAyersVirt, HOBOT_AYERS_VIRT_MACHINE)

struct HobotAyersVirt {
...
static void ayers_virt_set_emmc(Object *obj, bool value, Error **errp)
{
    HobotAyersVirt *s = HOBOT_AYERS_VIRT_MACHINE(obj);

    s->cfg.has_emmc = value;
}

static void fdt_create(HobotAyersVirt *s)
{
...
static void fdt_add_clk_node(HobotAyersVirt *s, const char *name,
...
static void fdt_add_cpu_nodes(HobotAyersVirt *s, uint32_t psci_conduit)
{
    int i;
    ...
    for (i = AYERS_SOC_NR_ACPUS - 1; i >= 0; i--) {
...
static void fdt_add_gic_nodes(HobotAyersVirt *s)
{
    char *nodename;
    ...
    qemu_fdt_setprop_cells(s->fdt, nodename, "interrupts",
                           GIC_FDT_IRQ_TYPE_PPI, AYERS_SOC_GIC_MAINT_IRQ,
...
static void fdt_add_timer_nodes(HobotAyersVirt *s)
{
    ...
    qemu_fdt_setprop_cells(s->fdt, "/timer", "interrupts",
            GIC_FDT_IRQ_TYPE_PPI, AYERS_SOC_TIMER_S_EL1_IRQ, irqflags,
            GIC_FDT_IRQ_TYPE_PPI, AYERS_SOC_TIMER_NS_EL1_IRQ, irqflags,
            GIC_FDT_IRQ_TYPE_PPI, AYERS_SOC_TIMER_VIRT_IRQ, irqflags,
            GIC_FDT_IRQ_TYPE_PPI, AYERS_SOC_TIMER_NS_EL2_IRQ, irqflags);
    qemu_fdt_setprop(s->fdt, "/timer", "compatible",
                     compat, sizeof(compat));
}

static void fdt_add_uart_nodes(HobotAyersVirt *s)
{
    uint64_t addrs[] = { MM_UART1, MM_UART0 };
    uint64_t size[] = {MM_UART1_SIZE, MM_UART0_SIZE};
    unsigned int irqs[] = { AYERS_SOC_UART1_IRQ_0, AYERS_SOC_UART0_IRQ_0 };
...
static void fdt_add_sdhci_nodes(HobotAyersVirt *s)
{
    ...
    for (i = ARRAY_SIZE(s->soc.cpu_subsys.peri.mmc) - 1; i >= 0; i--) {
        ...
        qemu_fdt_setprop_cells(s->fdt, name, "interrupts",
                               GIC_FDT_IRQ_TYPE_SPI, AYERS_SOC_SDHCI0_IRQ_0 + i * 2,
                               GIC_FDT_IRQ_FLAGS_LEVEL_HI);
...
static void fdt_add_memory_nodes(HobotAyersVirt *s, void *fdt, uint64_t ram_size)
{
...
static void ayers_virt_modify_dtb(const struct arm_boot_info *binfo,
                                    void *fdt)
{
    HobotAyersVirt *s = container_of(binfo, HobotAyersVirt, binfo);

    fdt_add_memory_nodes(s, fdt, binfo->ram_size);
}

static void *ayers_virt_get_dtb(const struct arm_boot_info *binfo,
                                  int *fdt_size)
{
    const HobotAyersVirt *board = container_of(binfo, HobotAyersVirt, binfo);

    *fdt_size = board->fdt_size;
    return board->fdt;
}

static void ayers_virt_machine_done(Notifier *notifier, void *data)
{
    HobotAyersVirt *s = container_of(notifier, HobotAyersVirt,
                                    machine_done);
...
static void ayers_virt_init(MachineState *machine)
{
    HobotAyersVirt *s = HOBOT_AYERS_VIRT_MACHINE(machine);
    ...
    object_initialize_child(OBJECT(machine), "ayers-virt", &s->soc,
                            TYPE_AYERS_SOC);
    ...
    s->binfo.get_dtb = ayers_virt_get_dtb;
    s->binfo.modify_dtb = ayers_virt_modify_dtb;
    ...
    arm_load_kernel(&s->soc.cpu_subsys.apu.cpu[0], machine, &s->binfo);
     ...
     if (!machine->kernel_filename) {
        s->machine_done.notify = ayers_virt_machine_done;
        qemu_add_machine_init_done_notifier(&s->machine_done);
     }
}

static void ayers_virt_machine_instance_init(Object *obj)
{
}

static void ayers_virt_machine_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);

    mc->desc = "Hobot Ayers Virtual Development Board";
    mc->init = ayers_virt_init;
    mc->min_cpus = AYERS_SOC_NR_ACPUS + AYERS_SOC_NR_RCPUS;
    mc->max_cpus = AYERS_SOC_NR_ACPUS + AYERS_SOC_NR_RCPUS;
    mc->default_cpus = AYERS_SOC_NR_ACPUS + AYERS_SOC_NR_RCPUS;
    mc->no_cdrom = true;
...
static const TypeInfo ayers_virt_machine_init_typeinfo = {
    .name       = TYPE_HOBOT_AYERS_VIRT_MACHINE,
    .parent     = TYPE_MACHINE,
    .class_init = ayers_virt_machine_class_init,
    .instance_init = ayers_virt_machine_instance_init,
    .instance_size = sizeof(HobotAyersVirt),
};

static void ayers_virt_machine_init_register_types(void)
{
    type_register_static(&ayers_virt_machine_init_typeinfo);
}

type_init(ayers_virt_machine_init_register_types)
